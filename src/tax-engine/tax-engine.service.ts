...
export class TaxEngineService {
  ...
  async resolve(dto: ResolveTaxDto) {
    ...
    if (!dto.tax_classification) { missing.push('tax_classification'); if (dto.nbs) { try { const result = await this.rtc.getClassificationsByNbs(dto.nbs, dto.effective_at); const cached = await this.domains.cacheRecord('rtc-classifications-by-nbs', `${dto.nbs}@${dto.effective_at.slice(0, 10)}`, result, 24); candidates = result.payload; sources.push({ ...cached, authority: 'runtime-reference' }); } catch (error) { warnings.push(`Could not fetch official NBS classification candidates: ${error instanceof Error ? error.message : String(error)}`); } } }
    else { try { const result = await this.rtc.getClassificationForNfse(dto.tax_classification, dto.effective_at); const cached = await this.domains.cacheRecord('rtc-nfse-cclasstrib', `${dto.tax_classification}@${dto.effective_at.slice(0, 10)}`, result, 24); officialClassification = result.payload; sources.push({ ...cached, authority: 'runtime-reference' }); } catch (error) { warnings.push(`Official cClassTrib validation failed: ${error instanceof Error ? error.message : String(error)}`); missing.push('verified_tax_classification'); } }
