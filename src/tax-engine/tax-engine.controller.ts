...
export class TaxEngineController {
  ...
  @Get('position/:companyId') @ApiBearerAuth() @UseGuards(ApiKeyGuard) @RequireScope('tax:read') getPosition(@Param('companyId') companyId: string, @Query('period') period: string, @CurrentTaxAgentAuth() auth?: TaxAgentAuthContext) { if (auth && auth.companyId !== companyId) throw new ForbiddenException('API key cannot operate another company'); if (!period) throw new BadRequestException('period query parameter is required'); return this.position.getPosition(companyId, period); }
