...
export class NfseNationalProvider implements FiscalProvider {
  ...
  private async issuePrepared(input: CanonicalInvoiceInput, operation: FiscalOperationContext): Promise<IssueResult> {
    ...
    if (record.environment !== input.environment) throw new FiscalEngineError('TA_PREPARED_DPS_ENVIRONMENT_MISMATCH', 'Prepared DPS environment does not match invoice environment', false);
    if (record.schema_id !== this.schemas.active(input.environment).id) throw new FiscalEngineError('TA_PREPARED_DPS_SCHEMA_STALE', `Prepared DPS schema ${record.schema_id} is not the active ${this.schemas.active(input.environment).id}`, false);
    ...
    if (existingDocument) {
      if (existingDocument.sha256 !== material.record.signed_xml_sha256) throw new FiscalEngineError('TA_PREPARED_DPS_DOCUMENT_MISMATCH', 'Invoice already contains a different signed DPS; refusing to transmit', false);
    } else {
  ...
  private async authorizedFromReconciliation(invoiceId: string, dpsId: string, response: NationalApiResponse, environment: CanonicalInvoiceInput['environment'], certificate: CertificateMaterial): Promise<IssueResult> { if (response.erros?.length) return this.rejection(response, dpsId); const accessKey = typeof response.chaveAcesso === 'string' ? response.chaveAcesso : undefined; if (!accessKey) throw new FiscalEngineError('NFSE_RECONCILIATION_INCOMPLETE', `SEFIN returned a DPS reconciliation response without chaveAcesso for ${dpsId}; refusing to retransmit until state is known`, true, this.client.sanitize(response)); const full = await this.client.getByAccessKey(environment, accessKey, certificate); return this.authorizedResponse(invoiceId, dpsId, full, accessKey); }
  private async authorizedResponse(invoiceId: string, dpsId: string, response: NationalApiResponse, fallbackAccessKey?: string): Promise<IssueResult> { if (response.erros?.length) return this.rejection(response, response.idDps ?? response.idDPS ?? dpsId); const accessKey = response.chaveAcesso ?? fallbackAccessKey; if (!accessKey) throw new FiscalEngineError('NFSE_RESPONSE_INCOMPLETE', `SEFIN response does not contain chaveAcesso for ${dpsId}`, true, this.client.sanitize(response)); const providerReference = response.idDps ?? response.idDPS ?? dpsId; const authorizedXml = this.client.decodeNfseXml(response); if (authorizedXml) await this.documents.save({ invoiceId, kind: 'nfse-authorized-xml', content: authorizedXml, contentType: 'application/xml', metadata: { accessKey, providerReference } }); return { status: 'authorized', provider: this.name, accessKey, providerReference, raw: this.client.sanitize(response) }; }
  ...
  private assertDpsConformance(environment: CanonicalInvoiceInput['environment']) {
    ...
    if (!conformance.verified) throw new FiscalEngineError('TA_DPS_BUILDER_UNVERIFIED', `DPS builder conformance is not verified for active schema ${this.schemas.active(environment).id}: ${conformance.reason}`, false, { environment, schema_id: this.schemas.active(environment).id, conformance_verified: false });
  }
