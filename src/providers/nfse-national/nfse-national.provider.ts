...
export class NfseNationalProvider implements FiscalProvider {
  ...
  async issue(input: CanonicalInvoiceInput, operation: FiscalOperationContext): Promise<IssueResult> {
    ...
    const response = await this.client.issue(input.environment, signedXml, certificate); const providerReference = response.idDps ?? response.idDPS;
    if (response.erros?.length) return this.rejection(response, providerReference);
    const authorizedXml = this.client.decodeNfseXml(response); if (authorizedXml) await this.documents.save({ invoiceId: operation.invoiceId, kind: 'nfse-authorized-xml', content: authorizedXml, contentType: 'application/xml', metadata: { accessKey: response.chaveAcesso, providerReference } });
