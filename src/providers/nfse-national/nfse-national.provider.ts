...
export class NfseNationalProvider implements FiscalProvider {
  ...
  private async issuePrepared(input: CanonicalInvoiceInput, operation: FiscalOperationContext): Promise<IssueResult> {
    if ((process.env.TAXAGENT_DPS_BUILDER_MODE ?? 'draft') !== 'verified') throw new FiscalEngineError('TA_DPS_BUILDER_UNVERIFIED', 'Prepared DPS transmission remains blocked until TAXAGENT_DPS_BUILDER_MODE=verified', false);
