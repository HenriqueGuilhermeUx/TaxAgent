...
import { GissRpsSequenceService } from './giss-rps-sequence.service';
...
export class GissProvider implements FiscalProvider {
  ...
  constructor(
    ...
    private readonly reconciliation: GissReconciliationService,
    private readonly artifacts: GissArtifactsService,
    private readonly signatures: GissSignatureService,
    private readonly sequences: GissRpsSequenceService,
  ...
  async issue(input: CanonicalInvoiceInput, operation: FiscalOperationContext): Promise<IssueResult> {
    ...
    const series = 'TA';
    const rpsNumber = String(await this.sequences.reserve(input.companyId, input.environment, series, operation.invoiceId));
    ...
    await this.artifacts.save(operation.invoiceId, 'giss_rps_xml', rps, { rps_number: rpsNumber, series });
