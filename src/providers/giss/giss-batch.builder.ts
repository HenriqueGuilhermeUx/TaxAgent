...
export function buildAbrasfLoteRps(input: GissBatchInput): string {
  ...
  const prestador = `<Prestador><CpfCnpj><Cnpj>${cnpj}</Cnpj></CpfCnpj>${input.municipalRegistration ? `<InscricaoMunicipal>${esc(input.municipalRegistration)}</InscricaoMunicipal>` : ''}</Prestador>`;
  return `<EnviarLoteRpsEnvio xmlns="http://www.abrasf.org.br/nfse.xsd"><LoteRps Id="LOTE${esc(input.batchNumber)}" versao="2.04"><NumeroLote>${esc(input.batchNumber)}</NumeroLote>${prestador}<QuantidadeRps>1</QuantidadeRps><ListaRps>${input.rpsXml}</ListaRps></LoteRps></EnviarLoteRpsEnvio>`;
}
