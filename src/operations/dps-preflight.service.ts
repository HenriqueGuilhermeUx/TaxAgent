...
export class DpsPreflightService {
  ...
  async validate(dto: CreateInvoiceDto) {
    ...
    if (!dto.tax_decision_id) throw new BadRequestException('DPS dry-run requires a resolved tax_decision_id so RTC fields are traceable to a persisted TaxAgent decision');
    ...
    const issuedAt = formatNfseDateTimeUtc(); const input = this.taxEngine.validate({ companyId: dto.company_id, environment: 'test', competence: dto.competence ?? issuedAt.slice(0, 10), issuedAt, customer: { taxId: dto.customer.tax_id, name: dto.customer.name, cityCode: dto.customer.city_code }, service });
    ...
    return { valid: true, transmitted: false, environment: 'test', tax_decision_id: dto.tax_decision_id, dps_id: built.id, preview_sequence: built.sequence, schema: this.schemas.active('test').id, unsigned_xml_sha256: hash(built.xml), signed_xml_sha256: hash(signed), certificate_fingerprint: certificate.fingerprint, builder_mode: process.env.TAXAGENT_DPS_BUILDER_MODE ?? 'draft', checked_at: new Date().toISOString(), note: 'The DPS was built from a persisted tax decision, XSD-validated, signed with the active A1 and XSD-validated again. No request was sent to SEFIN.' };
  }
  private assertLiveInputs(company: FiscalCompany, service: CanonicalService) { if (String(company.tax_regime ?? '').toLowerCase() !== 'regular') throw new BadRequestException('First homologation cycle supports tax_regime=regular only'); if (!service.nationalServiceCode || !/^\d{6}$/.test(service.nationalServiceCode)) throw new BadRequestException('national_service_code must be a 6-digit cTribNac'); if (!service.serviceLocationCityCode) throw new BadRequestException('service_location_city_code is required for dry-run'); if (!service.issTaxation || !service.issWithholding) throw new BadRequestException('iss_taxation and iss_withholding are required for dry-run'); if (!service.operationIndicator || !service.taxSituation || !service.taxClassification) throw new BadRequestException('cIndOp, CST and cClassTrib are required for dry-run'); }
}
