...
export class DpsPreflightService {
  ...
  async validate(dto: CreateInvoiceDto) {
    ...
    return { valid: true, transmitted: false, environment: 'test', dps_id: built.id, preview_sequence: built.sequence, schema: this.schemas.active('test').id, unsigned_xml_sha256: hash(built.xml), signed_xml_sha256: hash(signed), certificate_fingerprint: certificate.fingerprint, builder_mode: process.env.TAXAGENT_DPS_BUILDER_MODE ?? 'draft', checked_at: new Date().toISOString(), note: 'The DPS was built, XSD-validated, signed with the active A1 and XSD-validated again. No request was sent to SEFIN.' };
  }
