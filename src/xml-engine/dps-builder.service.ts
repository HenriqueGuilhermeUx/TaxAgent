...
export class DpsBuilderService {
  ...
  async build(input: CanonicalInvoiceInput, company: FiscalCompany): Promise<DpsBuildResult> {
    ...
    const doc = { DPS: { '@_xmlns': 'http://www.sped.fazenda.gov.br/nfse', '@_versao': '1.01', infDPS: {
      '@_Id': id, tpAmb: input.environment === 'production' ? 1 : 2, dhEmi: new Date().toISOString(), verAplic: 'TaxAgent_0.9', serie: series, nDPS: sequence, dCompet: new Date().toISOString().slice(0, 10), tpEmit: 1, cLocEmi: company.city_code,
      prest: { CNPJ: company.tax_id, ...(company.municipal_registration ? { IM: company.municipal_registration } : {}) },
      toma: { CNPJ: input.customer.taxId, xNome: input.customer.name },
