...
export class InvoicesRepository {
  ...
  async create(input: CanonicalInvoiceInput, idempotencyKey?: string, taxDecisionId?: string): Promise<InvoiceRecord> { const id = createId('inv'); try { const { rows } = await this.db.query<InvoiceRecord>(`INSERT INTO invoices(id, company_id, environment, status, idempotency_key, canonical_input, tax_decision_id) VALUES ($1,$2,$3,'queued',$4,$5::jsonb,$6) RETURNING *`, [id, input.companyId, input.environment, idempotencyKey ?? null, JSON.stringify(input), taxDecisionId ?? null]); return rows[0]; } catch (error) { const pgCode = typeof error === 'object' && error !== null && 'code' in error ? String((error as { code: unknown }).code) : undefined; if (pgCode === '23505' && idempotencyKey) { const existing = await this.findByIdempotency(input.companyId, idempotencyKey); if (existing) return existing; } throw error; } }
