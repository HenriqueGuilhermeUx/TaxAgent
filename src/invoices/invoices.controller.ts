...
export class InvoicesController {
  ...
  create(
    ...
    if (auth && auth.environment !== dto.environment) throw new ForbiddenException('API key environment does not match request environment');
    return this.invoices.create(dto, idempotencyKey);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get invoice state and fiscal ledger' })
  findOne(@Param('id') id: string, @CurrentTaxAgentAuth() auth?: TaxAgentAuthContext) {
    return this.invoices.findOneForCompany(id, auth?.companyId);
  }
}
