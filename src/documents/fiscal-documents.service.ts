...
export class FiscalDocumentsService {
  ...
  async latestContent(invoiceId: string, kind: string) { const { rows } = await this.db.query<{ id: string; sha256: string; content_type: string | null; content: Buffer; metadata: unknown }>('SELECT id, sha256, content_type, content, metadata FROM fiscal_documents WHERE invoice_id=$1 AND kind=$2 AND content IS NOT NULL ORDER BY created_at DESC LIMIT 1', [invoiceId, kind]); return rows[0] ?? null; }
