...
interface Source { id: string; status: string; officialUrl?: string }
...
async function main() {
  ...
  for (const key of ['production', 'test']) {
    ...
    const response = await fetch(source.officialUrl, { headers: { 'user-agent': 'TaxAgent-SchemaSync/0.6' } });
    ...
    const manifest = { sourceId: source.id, sourceUrl: source.officialUrl, downloadedAt: new Date().toISOString(), archiveSha256: zipSha256, dpsXsd, eventXsd, xsdFiles };
