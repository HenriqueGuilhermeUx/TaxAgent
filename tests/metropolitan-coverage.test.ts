...
test('metropolitan coverage keeps national participation separate from direct public issuer capability', async () => {
  ...
  const capabilities = {
    resolve: async (cityCode: string) => {
      ...
      if (index === 0) return { cityCode, environment: 'production', nationalStandard: true, nationalPublicIssuer: true, route: 'national-direct', provider: 'nfse-national', source: 'official-national-parameters', checkedAt: '2026-09-24T00:00:00Z' };
      if (index === 1) return { cityCode, environment: 'production', nationalStandard: true, nationalPublicIssuer: false, route: 'unknown', provider: 'unknown', source: 'official-national-parameters', checkedAt: '2026-09-24T00:00:00Z' };
      if (index === 2) return { cityCode, environment: 'production', nationalStandard: true, nationalPublicIssuer: false, route: 'municipal-provider', provider: 'giss', source: 'taxagent-observed-official-rejection', checkedAt: '2026-09-24T00:00:00Z' };
      return { cityCode, environment: 'production', nationalStandard: false, nationalPublicIssuer: false, route: 'unknown', provider: 'unknown', source: 'official-national-parameters', checkedAt: '2026-09-24T00:00:00Z' };
    },
  };
