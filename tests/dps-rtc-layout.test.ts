...
const service = { description: 'Serviço de software', amount: 1000, finalConsumption: '0' as const, operationIndicator: '123456', taxSituation: '000', taxClassification: '000001' };

test('builds the current NT004 IBS/CBS nesting used by DPS', () => {
  assert.deepEqual(buildIbsCbsGroup(service), { finNFSe: 0, indFinal: '0', cIndOp: '123456', indDest: 0, valores: { trib: { gIBSCBS: { CST: '000', cClassTrib: '000001' } } } });
});
...
test('refuses partial or malformed IBS/CBS classification', () => {
  assert.throws(() => buildIbsCbsGroup({ ...service, finalConsumption: undefined }), /must be supplied together/);
